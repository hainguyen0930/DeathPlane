...
bool init()
{
	...
	else
	{
		...
		//Create window
		gWindow = SDL_CreateWindow( "DeathPlane", SDL_WINDOWPOS_UNDEFINED, SDL_WINDOWPOS_UNDEFINED, SCREEN_WIDTH, SCREEN_HEIGHT, SDL_WINDOW_SHOWN );
		if( gWindow == NULL )
		{
...
bool loadMedia()
{
    ...
    if(!gTutorial.loadFromFile("Tutorial.jpg"))
    {
        printf( "Failed to load background menu texture!\n"  );
		success = false;
    }
    ...
    g_Sound_Menu = Mix_LoadMUS("BKmusic.mp3");
